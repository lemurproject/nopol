...
'''
Groups the sorted annotations by the WARC file that included the original pages.
...
class WarcOutput(object):
    ...
    def __init__(self, ds, outdir, dirlists):
        ...
        self.prev_dirname = None
        self.prev_fileno = None
    ...
    def add_ann(self, ann, line):
        ...
        if self.prev_dirname != dirname and self.prev_fileno != fileno:
            if self.fp != None:
                print 'Closing previous file: %s-%s' % (self.prev_dirname, self.prev_fileno)
                self.fp.close()

            print 'Opening new file for: %s' % ann
            self.fp = self.create_warc_out(ann)

        self.fp.write(line)
        self.prev_dirname, self.prev_fileno = ann
